...
namespace :populate do
    namespace :y2011 do
        year_name = "2011"
        yr_id = 10
        namespace :offense do
            desc "Import #{year_name} offensive stats from csv file"
            task :stats => [:environment] do

                file = "Offense#{year_name}.csv"
                CSV.foreach("#{Rails.root}/db/seed_data/stats/#{file}", :headers => false) do |row|
        ...
        namespace :defense do
            desc "Import #{year_name} defensive stats from csv file"
            task :stats => [:environment] do

                file = "Defense#{year_name}.csv"
                CSV.foreach("#{Rails.root}/db/seed_data/stats/#{file}", :headers => false) do |row|
